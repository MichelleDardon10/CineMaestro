...
function PlaylistPage() {
  ...
  const handleCreatePlaylist = () => {
    if (newPlaylistName) {
      axios
        .post('http://localhost:5174/playlist', { name: newPlaylistName , title: newPlaylistName, username: "test"})
        .then((response) => {
            axios
            .get('http://localhost:5174/playlist')
            .then((response) => {
      ...
      <ul>
        {playlists.map((playlist, index) => (
          <li key={index}>
            ...
            <button className="remove-button" onClick={() => handleRemovePlaylist(playlist.id)}>
              Remove
            </button>
          </li>
        ))}
